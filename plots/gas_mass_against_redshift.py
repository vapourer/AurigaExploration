...
class plot:
    def __init__(self,
        ...
        print('New gas_mass_against_redshift plot')  
    ...
    def create(self) -> None:
        ...
        for snapshot_number in snapshot_numbers:
            ...
            if subhalo > 0:
                ...
                mass.append((10**10) * np.max(grouped_by_snapshot_number[snapshot_number]['GasMass']) / h)

        plt.plot(redshift, mass, color='y')
        ...
        plt.xlabel('Redshift')
        plt.ylabel('Gas mass (in solar masses)')
        # plt.legend()
        plt.title(f'Gas mass against redshift for peak mass index {self.__peak_mass_index}')
        plt.savefig(f'./output/halo_6/gas/GasMassRedshift_{self.__peak_mass_index}.png')
